...
type MetricsResponse struct {
	...
	LastTtyRxTimeMs int64  `json:"lastTtyRxTimeMs"`
	LastTtyTxTimeMs int64  `json:"lastTtyTxTimeMs"`
	LastAtCmdTimeMs int64  `json:"lastAtCmdTimeMs"`
	LastConnTimeMs  int64  `json:"lastConnTimeMs"`
}
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		for _, m := range modems {
			...
			response := MetricsResponse{
				...
				LastTtyRxTimeMs: ternary(metrics.LastTtyRxTime.IsZero(), -1, int64(time.Since(metrics.LastTtyRxTime)/time.Millisecond)),
				LastTtyTxTimeMs: ternary(metrics.LastTtyTxTime.IsZero(), -1, int64(time.Since(metrics.LastTtyTxTime)/time.Millisecond)),
				LastAtCmdTimeMs: ternary(metrics.LastAtCmdTime.IsZero(), -1, int64(time.Since(metrics.LastAtCmdTime)/time.Millisecond)),
				LastConnTimeMs:  ternary(metrics.LastConnTime.IsZero(), -1, int64(time.Since(metrics.LastConnTime)/time.Millisecond)),
			}
